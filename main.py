...
async def init_db():
    try:
        ...
        logger.info("[Database] Async table creation completed.")
    except Exception as e:
        logger.error(f"[Database] Failed to initialize tables: {e}")

async def preseed_memory_defaults():
    """Pre-populates default project memories if the table is empty."""
    async with SessionLocal() as db:
        try:
            from sqlalchemy import func
            result = await db.execute(select(func.count(ProjectMemory.key)))
...
class ConnectionManager:
    ...
    async def broadcast(self, message: dict):
        ...
        await asyncio.gather(*(_safe_send(conn) for conn in connections), return_exceptions=True)
...
async def process_single_job(job_id: str):
    ...
    async with SessionLocal() as db:
        try:
            ...
            memories_result = await db.execute(select(ProjectMemory))
            ...
            # Shared short-term memory (stores worker outputs)
            short_term_memory = {}
            ...
            async def run_step(index: int, step: dict):
                ...
                logger.info(f"[Orchestrator] Step {index+1}: '{step_name}' -> routing to '{worker_type}'")
            ...
            # Identify steps for pipeline scheduling
            research_step = next(((i, s) for i, s in enumerate(plan_steps) if s["worker"] == "research_worker"), None)
            writer_step = next(((i, s) for i, s in enumerate(plan_steps) if s["worker"] == "script_writer"), None)
            publisher_step = next(((i, s) for i, s in enumerate(plan_steps) if s["worker"] == "publishing_worker"), None)
            
            # Parallel middle workers (e.g. lore_checker, thumbnail_strategist, voice_timing, style_consistency, fact_verifier)
            parallel_steps = [(i, s) for i, s in enumerate(plan_steps) if s["worker"] not in ("research_worker", "script_writer", "publishing_worker")]

            # Phase 1: Research
            if research_step:
                await run_step(*research_step)
                
            # Phase 2: Script Writing + Evaluation
            if writer_step:
                await run_step(*writer_step)
                
            # Phase 3: Parallel Workers Execution
            if parallel_steps:
                logger.info(f"[Orchestrator] Running Phase 3 workers concurrently: {[s['worker'] for _, s in parallel_steps]}")
                await asyncio.gather(*(run_step(i, s) for i, s in parallel_steps))
                
            # Phase 4: Publishing final assembly
            if publisher_step:
                await run_step(*publisher_step)
            ...
            try:
                # Reload to guarantee clean commit
                result = await db.execute(select(Job).where(Job.id == job_id))
                job = result.scalars().first()
                if job:
                    job.status = "failed"
...
async def background_job_processor_daemon():
    ...
    while True:
        try:
            async with SessionLocal() as db:
                result = await db.execute(select(Job).where(Job.status == "queued"))
...
async def trigger_harness_job(task: StructuredTask, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    """
    ...
    result = await db.execute(select(Series).where(Series.title == task.topic))
...
async def get_project_memory(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(ProjectMemory))
    memories = result.scalars().all()
    return [{"key": m.key, "value": m.value} for m in memories]

@app.post("/api/harness/memory")
async def save_project_memory(memory_item: ProjectMemorySchema, db: AsyncSession = Depends(get_db)):
    """Memory Layer: Save or update an editorial guideline setting."""
    result = await db.execute(select(ProjectMemory).where(ProjectMemory.key == memory_item.key))
...
async def delete_project_memory(key: str, db: AsyncSession = Depends(get_db)):
    """Memory Layer: Delete or reset a specific editorial memory setting."""
    result = await db.execute(select(ProjectMemory).where(ProjectMemory.key == key))
    mem = result.scalars().first()
    if not mem:
...
async def process_job_refinement(job_id: str, edited_slides: List[Any]):
    ...
    async with SessionLocal() as db:
        try:
            ...
            try:
                # Retrieve clean session to persist error state
                async with SessionLocal() as err_db:
                    res = await err_db.execute(select(Job).where(Job.id == job_id))
                    err_job = res.scalars().first()
                    if err_job:
                        err_job.status = "failed"
                        err_job.error_message = f"Refinement error: {e}"
                        await err_db.commit()
                        await broadcast_event("job_updated", {
                            "job_id": err_job.id,
                            "status": err_job.status,
                            "error_message": err_job.error_message
                        })
            except Exception as db_err:
                logger.error(f"[Refine Task] Failed to persist refinement error state: {db_err}")


@app.post("/api/jobs/{job_id}/refine")
async def refine_job_storyboard(job_id: str, req: RefineRequest, db: AsyncSession = Depends(get_db)):
    """
...
async def legacy_trigger(job_in: JobCreate, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
...
@app.get("/api/series")
async def list_series(skip: int = 0, limit: int = 50, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Series).offset(skip).limit(limit))
